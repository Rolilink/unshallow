...
import {
	MigrationResult,
	MigrationSummary,
} from '../langgraph-workflow/interfaces/shared-types.js';
...
export class ParallelMigrationManager {
	 ...
	 */
	private getRetryCountsDisplay(retries: {
		rtl: number;
		test: number;
		ts: number;
		lint: number;
	...
	async runAll(): Promise<MigrationSummary> {
		...
		const workers: Array<{
			promise: Promise<MigrationResult>;
			filePath: string;
		}> = [];
		...
		while (workers.length > 0) {
			...
			const completed = await Promise.race(
				workers.map(w =>
					w.promise.then(result => ({result, filePath: w.filePath})),
				),
			);
			...
			if (completed.result.success) {
				console.log(
					`✅ Migrated: ${completed.result.relativePath} ${retryInfo}`,
				);
			} else {
				console.log(
					`❌ Failed: ${completed.result.relativePath} ${
						completed.result.errorStep ? `(${completed.result.errorStep})` : ''
					} ${retryInfo}`,
				);
			}
	...
	private async cleanupAllUnshallowDirs(): Promise<void> {
		...
		for (const dir of unshallowDirs) {
			try {
				...
				process.stdout.write(
					`Error cleaning up directory ${dir}: ${
		...
		process.stdout.write(
			`Cleaned up ${cleanedCount} .unshallow ${
				cleanedCount === 1 ? 'directory' : 'directories'
			}\n`,
		);
	}
	 ...
	 */
	private createWorker(file: TestFileItem): {
		promise: Promise<MigrationResult>;
		filePath: string;
	} {
		const startTime = Date.now();
		...
		const promise = (async () => {
			try {
				...
				const retries = {
					...
					total:
						result.file.retries.rtl +
						result.file.retries.test +
						result.file.retries.ts +
						result.file.retries.lint,
				};
