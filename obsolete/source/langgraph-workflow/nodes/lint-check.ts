 ...
 */
export const lintCheckNode = async (
	state: WorkflowState,
): Promise<NodeResult> => {
	const {file} = state;
	...
	const attemptNumber =
		file.currentStep === WorkflowStep.LINT_CHECK_FAILED
			? logger.getAttemptCount('lint-fix')
			: file.retries.lint === 0
			? logger.incrementAttemptCount('lint')
			: logger.getAttemptCount('lint');

	await logger.logNodeStart(
		NODE_NAME,
		`Checking linting (attempt #${attemptNumber}): ${file.path}`,
	);
	...
	try {
		...
		try {
			...
			const combinedOutput = [errorOutput, checkStdout, checkStderr]
				.filter(Boolean)
				.join('\n');
			...
			await logger.error(
				NODE_NAME,
				`Lint check failed with ${errors.length} errors`,
			);
