...
class ContextEnricher {
   ...
   */
  async enrichContext(testFilePath: string, options?: EnrichmentOptions): Promise<EnrichedContext>;
}
...
export class ContextEnricher {
  ...
  async enrichContext(testFilePath: string, options: EnrichmentOptions = {}): Promise<EnrichedContext> {
    ...
    if (!testedComponent) {
      throw new Error(`Could not identify a component under test in ${testFilePath}`);
    }
    
    // Get component file path from import
    const componentFilePath = this.resolveImportPath(testedComponent.importPath);
    ...
    if (options.exampleTests && options.exampleTests.length > 0) {
      ...
      for (const examplePath of options.exampleTests) {
        try {
          ...
          context.exampleTests.set(path.relative(this.projectRoot, absolutePath), content);
        } catch (error) {
    ...
    if (options.extraContextFile) {
      try {
        ...
        console.warn(`Failed to load extra context file ${options.extraContextFile}:`, error);
      }
    }
  ...
  private findTestedComponent(sourceFile: ts.SourceFile): { name: string; importPath: string } | null {
    ...
    const testPatterns = [
      'shallow', 'mount', 'render',  // Enzyme and RTL
      'screen', 'findBy', 'getBy',   // RTL patterns
      'wrapper', 'querySelector'     // Common testing patterns
    ];
      ...
      if (ts.isCallExpression(node)) {
        ...
        if (testPatterns.some(pattern => callText.includes(pattern))) {
          if (node.arguments.length > 0) {
            ...
            if (ts.isJsxElement(firstArg) || ts.isJsxSelfClosingElement(firstArg)) {
            ...
            if (componentName) {
              ...
              const matchingImport = imports.find(imp => imp.name === componentName);
              if (matchingImport) {
                testedComponent = {
                  name: componentName,
    ...
    try {
      ...
      for (const importPath of imports) {
        ...
        try {
          ...
          relatedFiles.set(path.relative(this.projectRoot, resolvedPath), importContent);
          
          // Process nested imports if not at max depth
          if (currentDepth < maxDepth) {
            await this.processImports(resolvedPath, relatedFiles, currentDepth + 1, maxDepth);
          }
        } catch (error) {
          console.warn(`Error processing import ${importPath} from ${filePath}:`, error);
        }
      }
    } catch (error) {
  ...
  private resolveImportPath(importPath: string, basePath: string = this.projectRoot): string {
